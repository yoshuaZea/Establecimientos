...
export const removelistenersInputs = selector => {
    ...
    const alertas = ['border-l-4', 'border', 'border-danger']
...
export const validateForm = (array) => {
    // Clases
    const alert = ['d-block', 'text-danger', 'mt-1']
    const noValido = ['border-danger']
    ...
    for (let elem of array) {
        ...
        if(elem.classList.contains('required') || elem.classList.contains('radio')){
            ...
            elem.previousElementSibling.classList.remove('text-danger')
            ...
            if (elem.nodeName == "INPUT" && elem.type !== "file") {
                if(elem.value == ""){
                    if(document.querySelector(`[input="${elem.name}"]`)){
                        ...
                        elem.previousElementSibling.classList.add('text-danger')
                        elem.focus()
                        return false
                    }
